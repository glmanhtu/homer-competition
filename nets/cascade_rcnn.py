...
class CascadeRCNN(nn.Module):
    ...
    def forward(self, inp, targets=None):
        ...
        batch_target = list()
        batch_length = list()
        for idx, im in enumerate(inp):
            batch_img.append(im)
            valid_size.append(tuple(im.shape[1:]))
            if targets:
                target = torch.cat([targets[idx]['labels'][:, None], targets[idx]['boxes']], dim=-1)
                batch_target.append(target)
                batch_length.append(len(target))

        x = torch.stack(batch_img)
        if targets:
            targets = {"target": torch.cat(batch_target, dim=0), "batch_len": batch_length}
        xs = self.backbone(x)
        xs = self.fpn(xs)
